-- Taking the existing receive booking SELECT then adding more logics for overstock requested KPIs
...
processed_data AS (SELECT
    ...
    CASE
        ...
        WHEN ag.ZIEL_LHM LIKE 'KF_OV%' THEN 'Clarification' -- adding new clarification logic
...
WHERE
 ...
 -- Final Select pivoting the columns
 SELECT
 	date_only AS "Date",
    Shift AS "Shift",
    AP AS "Workstation",
    ...
    SUM(CASE WHEN Source_Channel = 'OSR' THEN Quantity ELSE 0 END) AS "Store returns outlet",
    SUM(CASE WHEN Source_Channel = 'OSR (OV)' THEN Quantity ELSE 0 END) AS "Store returns overstock",
    ...
    SUM(CASE WHEN Distribution_Channel = 'Clarification' THEN Quantity ELSE 0 END) AS "Clarification"
FROM 
    processed_data
GROUP BY 
    date_only,
