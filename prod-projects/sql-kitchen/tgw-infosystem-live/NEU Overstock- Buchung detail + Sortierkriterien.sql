-- TGW template - infosystems takes only a single parameter

WITH
 params AS (
 SELECT
 decode(:reflhmfilter, 'null', '') AS ref_lhm_filter,
 ...
 AND (
 p.ref_lhm_filter IS NULL
 OR (INSTR(p.ref_lhm_filter, '%') > 0 AND UPPER(t1.Reference_LHM) LIKE UPPER(p.ref_lhm_filter))
 OR (INSTR(p.ref_lhm_filter, ',') > 0 AND ',' || UPPER(p.ref_lhm_filter) || ',' LIKE '%,' || UPPER(t1.Reference_LHM) || ',%')
 OR (INSTR(p.ref_lhm_filter, '%') = 0 AND INSTR(p.ref_lhm_filter, ',') = 0 AND UPPER(t1.Reference_LHM) = UPPER(p.ref_lhm_filter))
 )
 UNION ALL
 -- Part 2: Dummy Goods
 SELECT
