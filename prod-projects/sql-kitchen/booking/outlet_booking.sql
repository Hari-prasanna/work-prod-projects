...
cte1 AS (
    ...
    SELECT
        ...
        DECODE(JSON_VALUE(hv.CUST_DATA, '$.QUALITYID_SEKTOR'), '1', 'A', '2', 'B', '3', 'C', '4', 'D') AS "Quality_Fallback", -- Fallback: Decodes directly from its own JSON data
...
-- 4. Deduplicate stock table to safely grab CTE2 Quality
cte2 AS (
    SELECT 
        LC, 
        "EANs", 
...
SELECT 
    ...
    t1."Category", 
    t1."Sort_ID",
    COALESCE(t2."Quality", t1."Quality_Fallback") AS "Quality", -- Checks cte2 first for BOTH datasets. If missing, uses the respective Quality_Fallback
    CASE WHEN t1.origin = 2 THEN 'Yes' ELSE 'No' END AS "Clarification", -- Flags Clarification based on dataset origin
